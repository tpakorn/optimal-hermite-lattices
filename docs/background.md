...
holds exactly whenever \(f\) is a polynomial up to some degree \(N\). In 1D, the optimal \(n\)-point Gauss–Hermite rule is exact for polynomials of degree up to \(2n - 1\), with nodes at the roots of the probabilist's Hermite polynomial \(\text{He}_n(\xi)\).
...
\[
\text{He}_n(\xi) = (-1)^n \, e^{\xi^2/2}\, \frac{d^n}{d\xi^n}\, e^{-\xi^2/2}
\]

and the orthogonality relation

\[
\int_{-\infty}^{\infty} \text{He}_m(\xi)\, \text{He}_n(\xi)\, \omega(\xi)\, d\xi = n!\, \delta_{mn}.
\]
...
\[
f^{(\text{eq})}(\boldsymbol{\xi}) = \omega(\boldsymbol{\xi}) \sum_{n=0}^{\infty} \frac{1}{n!}\, \mathbf{a}^{(n)} : \mathbf{H}^{(n)}(\boldsymbol{\xi})
\]
...
\[
f_a^{(\text{eq})} = w_a \left[ \rho + \rho\, \frac{\mathbf{u} \cdot \boldsymbol{\xi}_a}{c_s^2} + \frac{\rho}{2\, c_s^4}\left((\mathbf{u} \cdot \boldsymbol{\xi}_a)^2 - c_s^2\, |\mathbf{u}|^2\right) + \cdots \right]
\]
